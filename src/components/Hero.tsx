...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      if (reduced) {
        ...
        gsap.set("[data-h-img]", { clipPath: "inset(0 0 0 0 round 0 0 0 200px)" });
        return;
      }
      const tl = gsap.timeline({ defaults: { ease: "power3.out" } });
      tl.fromTo(
        "[data-h-img]",
        { clipPath: "inset(0 0 100% 0 round 0 0 0 200px)", scale: 1.15 },
        { clipPath: "inset(0 0 0% 0 round 0 0 0 200px)", scale: 1.05, duration: 1.8, ease: "power4.inOut" },
        0.2,
      )
      ...
      // Scroll: image zooms out, text moves at a different speed
      ...
      // Cursor parallax on the image (fine pointers only)
  ...
  return (
    <section id="home" ref={root} className="relative isolate overflow-hidden bg-ivory pb-28 pt-24 lg:pb-40 lg:pt-0">
      {/* Background shapes */}
      ...
      <div className="relative mx-auto max-w-[1440px] lg:min-h-[100svh]">
        {/* Image — occupies ~58% on desktop, overlapped by text */}
        <div
          data-h-img
          className="relative ml-auto mr-0 mt-2 aspect-[4/5] w-[92%] overflow-hidden rounded-bl-[200px] sm:aspect-[5/4] lg:absolute lg:inset-y-0 lg:right-0 lg:mt-0 lg:aspect-auto lg:h-full lg:w-[58%] lg:rounded-bl-[200px]"
          style={{ clipPath: "inset(0 0 100% 0 round 0 0 0 200px)" }}
        >
          <img
            src={IMAGES.hero}
            ...
            fetchPriority="high"
          />
          <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(247,248,245,0.85)_0%,rgba(247,248,245,0)_35%)] hidden lg:block" />
          <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(47,111,107,0)_50%,rgba(47,111,107,0.25)_100%)]" />

          {/* Floating badge on image */}
          <div
            data-h-float
            className="glass absolute bottom-6 right-5 z-10 rounded-2xl px-4 py-3 opacity-0 shadow-soft sm:bottom-8 sm:right-8"
          >
            <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-teal-700">Anytime.</p>
            <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-coral">Anywhere.</p>
          </div>
        </div>
        ...
        <div
          data-h-copy
          className="relative px-5 pt-8 sm:px-8 sm:pt-12 lg:mt-0 lg:flex lg:min-h-[100svh] lg:w-[58%] lg:flex-col lg:justify-center lg:px-12 lg:pt-24"
        >
          <div className="lg:max-w-[720px]">
            ...
            <h1 className="display mt-6 text-[clamp(3.6rem,10.5vw,9.5rem)] text-ink">
            ...
            <p data-h-p className="mt-6 max-w-[520px] text-[16px] leading-relaxed text-muted opacity-0 sm:text-[17.5px]">
          ...
          <div
            data-h-float
            className="glass mt-8 inline-flex w-fit flex-wrap items-center gap-x-6 gap-y-3 rounded-full px-5 py-3 opacity-0 shadow-soft lg:mt-12"
          >
            {STATUS.map(({ label, Icon }, i) => (
              <span key={label} className="flex items-center gap-2.5 text-[13px] font-semibold text-ink">
                <span className={`grid h-7 w-7 place-items-center rounded-full ${i === 0 ? "bg-coral text-white" : "bg-teal-700/10 text-teal-700"}`}>
